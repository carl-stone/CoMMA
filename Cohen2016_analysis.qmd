...
cohen_combined_long |> 
  mutate(variable = case_when(
    Site %in% variable_sites ~ "varies",
    !(Site %in% variable_sites) ~ "not_varies")) |> 
  group_by(Site, Strand, Motif, time, group, variable) |> 
  summarise(beta = mean(beta),
            cov = mean(cov)) |> 
  ggplot(aes(time, methylBtoM(beta))) +
#  facet_wrap(~ variable) +
...
cohen_combined_long %>%
  filter(group != 't0') %>%
  rowwise() %>% 
  mutate(m = methylBtoM(beta)) %>%
  group_by(Site, time, group) %>%
  nest() %>%
  spread(key = group, value = data) %>%
  mutate()

#constant_sites <- cohen_combined_long |> 
#  filter(group != 't0', Site <= 100000) |> 
#  rowwise() |> 
#  mutate(m = methylBtoM(beta)) |> 
#  group_by(Site, time, group) |> 
#  summarise(sd = sd(m),
...
#              values_from = c(m, sd)) |> 
#  rowwise() |> 
#  mutate(dif = m_Amp - m_LB) |> 
#  filter(dif == 0 & (sd_Amp == 0 | sd_LB == 0)) |> 
#  dplyr::rename(c(Site_constant = Site,
#                  time_constant = time))

constant_sites <- cohen_combined_long |> 
  dplyr::filter(group != 't0') |> 
  rowwise() |> 
  mutate(m = methylBtoM(beta)) |> 
  group_by(Site, time, group) |> 
  # Need to do this to remove constant rows because t.test has no method for
  # ignoring these errors
  dplyr::filter(length(unique(m)) == 1) |> 
  dplyr::rename(c(Site_constant = Site,
                  time_constant = time))

constant_sites <- cohen_combined_long |> 
  dplyr::filter(group != 't0', Site < 1000000) |> 
  rowwise() |> 
  mutate(m = methylBtoM(beta)) |> 
  group_by(Site, time) |> 
  # Need to do this to remove constant rows because t.test has no method for
...
if (is.null(y)) {
  ...
  df <- nx - 1
  stderr <- sqrt(vx/nx)
  if (stderr < 10 * .Machine$double.eps * abs(19.93445)) 
...
constant_sites <- cohen_combined_long |> 
  ...
  # Need to do this to remove constant rows because t.test has no method for
  # ignoring these errors
  summarise(sterr = sqrt(var(m)/length(m)),
            m = mean(m),
            var_cutoff = 10 * .Machine$double.eps * abs(19.93445)) |> 
  dplyr::filter(sterr <= var_cutoff | length(unique(m)) == 1) |> 
  tidyr::unite("site_time_constant", c(Site, time), remove = FALSE)

#Currently the filter step correctly chooses sites but not times because times
#are not unique, so all are selected
# Need to find out how to filter by two variables
cohen_ttests <- cohen_combined_long |> 
  dplyr::filter(group != 't0') |> 
  #dplyr::slice(1:105) |> 
  rowwise() |> 
  mutate(m = methylBtoM(beta)) |>
  tidyr::unite("site_time", c(Site, time), remove = FALSE) |> 
  dplyr::filter(!(site_time %in% constant_sites$site_time_constant)) |> 
  group_by(Site, time) |> 
  t_test(formula = m ~ group,
         p.adjust.method = "fdr",
         var.equal = FALSE)

View(cohen_combined_long |> 
       dplyr::filter(group != 't0') |> 
       dplyr::slice(796))
...
cohen_combined_long |> 
  ...
  mutate(m = methylBtoM(beta)) |>
  group_by(Site, time) |> 
  mutate(sterr = sqrt(var(m)/length(m))) |> 
  ggplot(aes(x = sterr)) +
  geom_density() +
...
cohen_ttests <- cohen_combined_long |> 
  ...
  rowwise() |> 
  mutate(m = methylBtoM(beta))
cohen_ttests$constant <- 0
for (i in 1:nrow(constant_sites)) {
  cohen_ttests[cohen_ttests$Site == constant_sites[[i, 1]] &
                 cohen_ttests$time == constant_sites[[i, 2]],]$constant <- 1
}
cohen_ttests <- cohen_ttests |> 
  filter(constant == 0) |> 
  group_by(Site, time)

cohen_stats <- t_test(data = cohen_ttests,
                       formula = m ~ group,
                       p.adjust.method = "fdr",
                       var.equal = FALSE)
...
```{r compare_my_sites_to_cohen}
cohen_sites <- c(622228,
                 622229,
                 1031823,
                 1031824,
                 1100200,
...
all_samples_long |> 
  mutate(variable = case_when(
    Position %in% cohen_sites ~ "variable",
    !(Position %in% cohen_sites) ~ "not_variable")) |> 
  ggplot(aes(y = m)) +
  facet_wrap(~ genotype) +
...
cohen_combined_long |> 
  mutate(variable = case_when(
    Site %in% cohen_sites ~ "variable",
    !(Site %in% cohen_sites) ~ "not_variable"
  )) |> 
  ggplot(aes(y = methylBtoM(beta))) +
  geom_boxplot(aes(color = variable)) +
  theme_classic()
