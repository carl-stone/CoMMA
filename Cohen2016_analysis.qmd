...
cohen_ttests <- cohen_combined_long |> 
  dplyr::filter(group != 't0', Site < 100000) |> 
  rowwise() |> 
  mutate(m = methylBtoM(beta)) |> 
  dplyr::filter((Site %in% constant_sites$Site_constant &
                    time %in% constant_sites$time_constant))
...
cohen_ttests <- t_test(data = cohen_ttests,
                       formula = beta ~ group,
                       p.adjust.method = "fdr",
                       var.equal = FALSE)
...
View(cohen_ttests |> filter(Site %in% cohen_sites))
View(cohen_ttests |> 
  filter(Site %in% cohen_sites) |> 
  dplyr::select(Site, time, p) |> 
