...
| `test-loadAnnotation.R` | GFF3/BED parsing, feature_type filtering, feature_subtype preservation from GFF3 attribute |
| `test-findMotifSites.R` | Motif search, both strands, palindromic motifs |
...
| `test-coverageDepth.R` | `coverageDepth()` windowing, log2_transform, method, error handling — ~11 tests |
| `test-varianceByDepth.R` | `varianceByDepth()` bins, mod_type filtering, NA variance, coverage_bins=NULL — ~10 tests |
...
| `test-plot_methylation_distribution.R` | `plot_methylation_distribution()`: returns ggplot, mod_type filter, per_sample |
...
| `test-mValues.R` | `mValues()`: formula, NA/zero-coverage propagation, alpha, mod_type — ~24 tests |
...
| `test-enrichMethylation.R` | `.siteToGeneMap()`, `.computeGeneScores()`, `.parseTargetGenes()`, `.parseRegulatorGenes()`, `.extractGeneRoles()`, ORA+GSEA, gene_role, multi-feature_type, data.frame input — ~60 tests |
